 ...
 * 
 */
public class SolarMsgDecoder extends CumulativeProtocolDecoder {

	private static final Logger logger = LoggerFactory.getLogger(SolarMsgDecoder.class);

	public SolarMsgDecoder() {

	}

	/**
	 * flag(1 byte)+length(4 byte,后边内容的长度)+protocol code(4 byte)+content
	 * length的长度包括 ：消息号+ 内容 解码
	 */
	@Override
	protected boolean doDecode(IoSession session, IoBuffer iobuffer, ProtocolDecoderOutput protocolDecoderOutput)
			throws Exception {
		...
		synchronized (iobuffer) {
			...
			if (flag == 1) {
				...
				if (iobuffer.remaining() >= length) {
					...
					/**
					 * 这行代码有一个bug， 读取协议内容时，如果第一个字节不是1，则越过此字节继续往后的读，直到读到1，
