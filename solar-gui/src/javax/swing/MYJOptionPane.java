...
public class MYJOptionPane extends JOptionPane {
	...
	public static String showInputDialog(Component parentComponent, Object message, Object initialSelectionValue) {
		return (String) showInputDialog(parentComponent, message,
				UIManager.getString("OptionPane.inputDialogTitle", parentComponent), QUESTION_MESSAGE, null, null,
	...
	/**
	 * Prompts the user for input in a blocking dialog where the initial selection,
	 * possible selections, and all other options can be specified. The user will
	 * able to choose from <code>selectionValues</code>, where <code>null</code>
	 * implies the user can input whatever they wish, usually by means of a
	 * <code>JTextField</code>. <code>initialSelectionValue</code> is the initial
	 * value to prompt the user with. It is up to the UI to decide how best to
	 * represent the <code>selectionValues</code>, but usually a
	...
	public static Object showInputDialog(Component parentComponent, Object message, String title, int messageType,
		...
		MYJOptionPane pane = new MYJOptionPane(message, messageType, OK_CANCEL_OPTION, icon, null, null);
	...
	public static void showMessageDialog(Component parentComponent, Object message) throws HeadlessException {
		showMessageDialog(parentComponent, message,
				UIManager.getString("OptionPane.messageDialogTitle", parentComponent), INFORMATION_MESSAGE);
	}
	 ...
	 * with the title, <b>Select an Option</b>.
	 *
	 * @param parentComponent
	 *            determines the <code>Frame</code> in which the dialog is
	 *            displayed; if <code>null</code>, or if the
	 ...
	 * <code>messageType</code> parameter is primarily used to supply a default icon
	 ...
	 * <p>
	 * The <code>messageType</code> parameter is primarily used to supply a default
	 ...
	 * <code>parentComponent</code>'s frame. <code>title</code> is the title of the
	 * returned dialog. The returned <code>JDialog</code> will not be resizable by
	 ...
	 * <code>JDialog</code> will be set up such that once it is closed, or the user
	 * clicks on one of the buttons, the optionpane's value property will be set
	 * accordingly and the dialog will be closed. Each time the dialog is made
	 * visible, it will reset the option pane's value property to
	 * <code>JOptionPane.UNINITIALIZED_VALUE</code> to ensure the user's subsequent
	...
	/**
	 * Creates and returns a new parentless <code>JDialog</code> with the specified
	 * title. The returned <code>JDialog</code> will not be resizable by the user,
	 * however programs can invoke <code>setResizable</code> on the
	 * <code>JDialog</code> instance to change this property. The returned
	 * <code>JDialog</code> will be set up such that once it is closed, or the user
	 * clicks on one of the buttons, the optionpane's value property will be set
	 * accordingly and the dialog will be closed. Each time the dialog is made
	...
	/**
	 * Brings up an internal dialog panel that displays a message using a default
	...
	/**
	 * Brings up an internal dialog panel with the options <i>Yes</i>, <i>No</i> and
	...
	/**
	 * Brings up a internal dialog panel where the number of choices is determined
	 ...
	 *            <code>String</code> object is rendered as a string; other objects
	...
	/**
	 * Brings up an internal dialog panel where the number of choices is determined
	...
	/**
	 * Brings up an internal dialog panel with a specified icon, where the number of
	 ...
	 * @param parentComponent
	 *            determines the <code>Frame</code> in which the dialog is
	 *            displayed; if <code>null</code>, or if the parentComponent has no
	 *            Frame, a default <code>Frame</code> is used
	 * @param message
	 *            the object to display in the dialog; a <code>Component</code>
	 *            object is rendered as a <code>Component</code>; a
	 *            <code>String</code> object is rendered as a string; other objects
	...
	/**
	 * Brings up an internal dialog panel with a specified icon, where the initial
	 * choice is determined by the <code>initialValue</code> parameter and the
	...
	public static int showInternalOptionDialog(Component parentComponent, Object message, String title, int optionType,
		...
		JOptionPane pane = new MYJOptionPane(message, messageType, optionType, icon, options, initialValue);
		 ...
		 * necessary for JApplets, because because an applet normally isn't made visible
		 * until after its start() method returns -- if this method is called from
		 * start(), the applet will appear to hang while an invisible modal frame waits
		 * for input.
		 */
	...
	/**
	 * Prompts the user for input in a blocking internal dialog where the initial
	 ...
	 * means of a <code>JTextField</code>. <code>initialSelectionValue</code> is the
	 * initial value to prompt the user with. It is up to the UI to decide how best
	 * to represent the <code>selectionValues</code>, but usually a
	 * <code>JComboBox</code>, <code>JList</code>, or <code>JTextField</code> will
	 * be used.
	 *
	 * @param parentComponent
	...
	public static Object showInternalInputDialog(Component parentComponent, Object message, String title,
		...
		JOptionPane pane = new MYJOptionPane(message, messageType, OK_CANCEL_OPTION, icon, null, null);
		pane.putClientProperty(PopupFactory_FORCE_HEAVYWEIGHT_POPUP, Boolean.TRUE);
		Component fo = KeyboardFocusManager.getCurrentKeyboardFocusManager().getFocusOwner();
	...
	/**
	 * Creates and returns an instance of <code>JInternalFrame</code>. The internal
	 * frame is created with the specified title, and wrapping the
	 * <code>JOptionPane</code>. The returned <code>JInternalFrame</code> is added
	 * to the <code>JDesktopPane</code> ancestor of <code>parentComponent</code>, or
	 * components parent if one its ancestors isn't a <code>JDesktopPane</code>, or
	 * if <code>parentComponent</code> doesn't have a parent then a
	 ...
	 *                if <code>parentComponent</code> does not have a valid parent
	 */
	public JInternalFrame createInternalFrame(Component parentComponent, String title) {
		Container parent = MYJOptionPane.getDesktopPaneForComponent(parentComponent);
		...
		addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent event) {
				...
				// (newValue = null in that case). Otherwise, close the dialog.
				if (iFrame.isVisible() && event.getSource() == MYJOptionPane.this
						&& event.getPropertyName().equals(VALUE_PROPERTY)) {
					// Use reflection to get Container.stopLWModal().
					try {
						Method method = AccessController
								.doPrivileged(new ModalPrivilegedAction(Container.class, "stopLWModal"));
						if (method != null) {
	 ...
	 *         <code>null</code> if the component is <code>null</code> or does not
	 *         have an ancestor that is a <code>JInternalFrame</code>
	 */
	...
	/**
	 * Returns the <code>Frame</code> to use for the class methods in which a frame
	 * is not provided.
	 *
	...
	/**
	 * Returns the name of the UI class that implements the {@literal L&F} for this
	 * component.
	 *
	 ...
	 * implies the user has not yet made a choice, <code>null</code> means the user
	 * closed the window with out choosing anything. Otherwise the returned value
	 * will be one of the options defined in this object.
	 *
	...
	/**
	 * Sets the options this pane displays. If an element in <code>newOptions</code>
	 * is a <code>Component</code> it is added directly to the pane, otherwise a
	 * button is created for the element.
	 *
	 * @param newOptions
	 ...
	 *
	 * @see #getInitialValue
	 * @beaninfo preferred: true bound: true description: The option pane's initial
	 *           value object.
	 */
	...
	/**
	 * Sets the option pane's message type. The message type is used by the Look and
	 * Feel to determine the icon to display (if not supplied) as well as
	 ...
	 * 
	 * @see #getMessageType
	 * @beaninfo preferred: true bound: true description: The option pane's message
	 *           type.
	 */
	...
	/**
	 * Sets the options to display. The option type is used by the Look and Feel to
	 * determine what buttons to show (unless options are supplied).
	 * 
	 * @param newType
	 *            an integer specifying the options the {@literal L&F} is to
	 *            display: <code>DEFAULT_OPTION</code>, <code>YES_NO_OPTION</code>,
	 *            <code>YES_NO_CANCEL_OPTION</code>, or
	 *            <code>OK_CANCEL_OPTION</code>
	 * @exception RuntimeException
	 *                if <code>newType</code> is not one of the legal values listed
	...
	/**
	 * Sets the input selection values for a pane that provides the user with a list
	 * of items to choose from. (The UI provides a widget for choosing one of the
	 * values.) A <code>null</code> value implies the user can input whatever they
	 * wish, usually by means of a <code>JTextField</code>.
	 * <p>
	 * Sets <code>wantsInput</code> to true. Use
	 * <code>setInitialSelectionValue</code> to specify the initially-chosen value.
	 * After the pane as been enabled, <code>inputValue</code> is set to the value
	 ...
	 * @beaninfo bound: true description: The option pane's initial selection value
	 *           object.
	 */
	 ...
	 * <code>wantsInput</code> is true. Note that this method is invoked internally
	 * by the option pane (in response to user action) and should generally not be
	 * called by client programs. To set the input value initially displayed as
	 * selected to the user, use <code>setInitialSelectionValue</code>.
	 *
	 * @param newValue
	 ...
	 * Default is to return <code>Integer.MAX_VALUE</code>. The value can be changed
	...
	/**
	 * Sets the <code>wantsInput</code> property. If <code>newValue</code> is true,
	 * an input component (such as a text field or combo box) whose parent is
	 * <code>parentComponent</code> is provided to allow the user to input a value.
	 * If <code>getSelectionValues</code> returns a non-<code>null</code> array, the
	 * input value is one of the objects in that array. Otherwise the input value is
	 ...
	 * @see #setSelectionValues
	 * @see #setInputValue
	 * @beaninfo preferred: true bound: true description: Flag which allows the user
	 *           to input a value.
	 */
	 ...
	 * initial value. This method should be invoked after the window containing the
	 * option pane is made visible.
	 */
	...
	/**
	 * Returns a string representation of this <code>JOptionPane</code>. This method
	 ...
	 * <code>AccessibleJOptionPane</code>. A new <code>AccessibleJOptionPane</code>
	...
	/**
	 * This class implements accessibility support for the <code>JOptionPane</code>
	 * class. It provides an implementation of the Java Accessibility API
	 * appropriate to option pane user-interface elements.
	 * <p>
